...
- [`inbox`](https://pkg.go.dev/github.com/rahmadafandi/fibr/inbox) — idempotent-consumer dedup: `Once(ctx, db, messageID, fn)` records a marker and runs the handler in one transaction so a message is processed exactly once under at-least-once delivery. Consumer-side complement to `outbox`.
