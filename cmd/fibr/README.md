...
Generate a ready-to-run [Fiber](https://gofiber.io/) project wired with [fibr](https://github.com/rahmadafandi/fibr).
...
```bash
fibr new myapp --module github.com/me/myapp --db postgres --layout ddd
```
...
| `--helpers-version` | `latest` | fibr version pinned in go.mod |
| `--local` | | replace fibr with a local path (development) |
...
Generated projects depend on `github.com/rahmadafandi/fibr`. Until that module is published to a proxy, use `--local /path/to/fibr` (adds a `replace`) or `--no-tidy` and wire the dependency manually.
